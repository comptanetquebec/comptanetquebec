// app/formulaire-fiscal-t2/envoyer-dossier/page.tsx
"use client";
 ...
 */
const SEND_ROUTE = "/formulaire-fiscal-t2/envoyer-dossier";
const UPLOAD_ROUTE = "/formulaire-fiscal-t2/depot-documents";
...
export default function EnvoyerDossierT2Page() {
  const params = useSearchParams();

  const fid = (params.get("fid") || "").trim();

  // ✅ type forcé pour cette route
  const type = "t2";
  ...
  return (
    <RequireAuth lang={lang} nextPath={nextPath}>
      {(userId) => <EnvoyerDossierT2Inner userId={userId} fid={fid} lang={lang} />}
    </RequireAuth>
  );
}

function EnvoyerDossierT2Inner({ userId, fid, lang }: { userId: string; fid: string; lang: Lang }) {
  ...
  const startCheckout = useCallback(async () => {
    ...
    try {
      ...
      const { data: d, error: e1 } = await supabase.from(DOCS_TABLE).select("id").eq("formulaire_id", fid).limit(1);
  ...
  const goBackUpload = useCallback(() => {
    router.push(withLang(UPLOAD_ROUTE, lang, { fid, type: "t2" }));
  }, [router, lang, fid]);
          ...
          <div className="ff-brand">
            ...
            <div className="ff-brand-text">
              ...
              <span>
                {t(
                  lang,
                  "Étape 3/3 — Paiement & envoi (T2)",
                  "Step 3/3 — Payment & submission (T2)",
                  "Paso 3/3 — Pago y envío (T2)"
                )}
              </span>
            </div>
          </div>
          ...
          <div className="ff-grid2">
            <div className="ff-field">
              ...
              <div className="ff-empty" style={{ borderStyle: "solid" }}>
                {loadingForm ? t(lang, "Chargement…", "Loading…", "Cargando…") : cqId || t(lang, "Non disponible", "Not available", "No disponible")}
              </div>
            </div>
        ...
        <section className="ff-card">
          <div className="ff-card-head">
            ...
            <p>
              {t(
                lang,
                "Votre dossier T2 sera automatiquement soumis et pris en charge.",
                "Your T2 file will be automatically submitted and processed.",
                "Su expediente T2 se enviará automáticamente y será procesado."
              )}
            </p>
          </div>
