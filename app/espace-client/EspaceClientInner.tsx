...
const I18N: Record<
  Lang,
  {
    title: string; intro_login: string; intro_signup: string;
    email: string; password: string; see: string; hide: string;
    login: string; signup: string; logging: string; signing: string;
    forgot: string; needAccount: string; haveAccount: string;
    createAccount: string; signIn: string; pwRule: string;
    loggedAs: (m: string) => string; resetSent: string; resetNeedEmail: string;
    pwTooShort: string; emailNotConfirmed: string; invalidCreds: string; rateLimit: string;
  }
> = {
  fr: {
    title: "Espace client",
    ...
    email: "Courriel", password: "Mot de passe", see: "Voir", hide: "Cacher",
    login: "Se connecter", signup: "Créer mon compte",
    logging: "Connexion…", signing: "Création…",
    forgot: "Mot de passe oublié ?", needAccount: "Pas de compte ?", haveAccount: "Déjà inscrit ?",
  ...
  en: {
    ...
    email: "Email", password: "Password", see: "Show", hide: "Hide",
    login: "Log in", signup: "Sign up", logging: "Logging in…", signing: "Signing up…",
    forgot: "Forgot password?", needAccount: "No account?", haveAccount: "Already registered?",
  ...
  es: {
    ...
    email: "Correo electrónico", password: "Contraseña", see: "Ver", hide: "Ocultar",
    login: "Iniciar sesión", signup: "Crear cuenta",
    logging: "Conectando…", signing: "Creando…",
    forgot: "¿Olvidaste tu contraseña?", needAccount: "¿No tienes cuenta?",
    haveAccount: "¿Ya tienes cuenta?", createAccount: "Crear cuenta", signIn: "Iniciar sesión",
...
export default function EspaceClientInner() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (error) setErrorMsg(mapAuthError(error.message, t));
      ...
      if (!data.session) {
        setInfo("Compte créé. Vérifiez votre boîte de réception pour confirmer votre courriel.");
        setMode("login");
  ...
  async function handleForgot() {
    setLoading(true); setInfo(null); setErrorMsg(null);
    const e = email.trim();
    if (!e) { setLoading(false); setErrorMsg(t.resetNeedEmail); return; }
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-slate-100 px-4">
        <div className="w-full max-w-md bg-white shadow-xl rounded-2xl p-8">
          ...
          <button onClick={handleLogout} className="mt-6 inline-flex items-center justify-center rounded-lg border px-4 py-2 text-sm hover:bg-slate-50">
            Logout
          </button>
        </div>
      </main>
      ...
      <div className="w-full max-w-md bg-white shadow-xl rounded-2xl p-8">
        ...
        <form onSubmit={mode === "login" ? handleLogin : handleSignup} className="mt-6 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border px-4 py-2 outline-none focus:ring-2 focus:ring-cq-primary"
          ...
          <div>
            ...
            <div className="flex items-stretch gap-2">
              <input
                type={showPwd ? "text" : "password"} required minLength={mode === "signup" ? 8 : 1}
              ...
              <button type="button" onClick={() => setShowPwd((s) => !s)}
                className="rounded-lg border px-3 text-sm hover:bg-slate-50" aria-pressed={showPwd}>
                {showPwd ? t.hide : t.see}
              </button>
            </div>
            {mode === "signup" && <p className="mt-1 text-xs text-slate-500">{t.pwRule}</p>}
          </div>

          <button type="submit" disabled={loading}
            className="w-full rounded-lg bg-cq-primary hover:bg-cq-primaryDark text-white font-semibold py-2 transition disabled:opacity-60">
            {loading ? (mode === "login" ? t.logging : t.signing) : (mode === "login" ? t.login : t.signup)}
          </button>

          {mode === "login" && (
            <button type="button" onClick={handleForgot}
              className="text-sm text-cq-primary hover:underline" disabled={loading}>
              {t.forgot}
            </button>
          )}
        ...
        <div className="mt-6 text-sm text-slate-700">
          {mode === "login" ? (
            <>
              {t.needAccount}{" "}
              <button className="text-cq-primary hover:underline"
                onClick={() => { setMode("signup"); setInfo(null); setErrorMsg(null); }}>
                {t.createAccount}
              </button>
            </>
          ) : (
            <>
              {t.haveAccount}{" "}
              <button className="text-cq-primary hover:underline"
                onClick={() => { setMode("login"); setInfo(null); setErrorMsg(null); }}>
                {t.signIn}
              </button>
            </>
          )}
      ...
      {(["fr", "en", "es"] as Lang[]).map((l) => (
        <button
          ...
          className={`px-2 py-1 rounded-md border text-xs ${
            lang === l ? "bg-cq-primary text-white border-cq-primary" : "hover:bg-slate-50"
          }`}
          type="button"
          aria-pressed={lang === l}
        >
